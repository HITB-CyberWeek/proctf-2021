...
flag = GenRS(31)+"="  #"0102030405060708090a0b0c0d0f000="
init_flag = flag
...
def GenerateFormula(password,to_size):
    ...
    for i in range(to_size):
        ...
        cells_z3.append(eval("password_z3[%d] %s password_z3[%d]" % (takes[i],op[0],ridx)))
...
for i in range(len(flag)):
	...
	#if val >0:
	flag2.append(val)
	flag2_z3.append(val ^ cells_z3[i])
	#else:
...
comp_p = ''
for i in range(8):
    comp_p+=chr(solv.model()[password_z3[i]].as_long())
print(password,comp_p)
for k in range(len(password_z3)):
    solv.add(password_z3[k] != ord(comp_p[k]))
